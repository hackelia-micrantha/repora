...
Each discovered `.github/workflows/*.yml` or `.yaml` file is parsed as data without executing it. The inventory records:

- workflow-level declared permissions;
- job-level declared permissions;
- use of `pull_request_target`;
- literal/dynamic runner labels;
- whether self-hosted execution is observed, not observed, or unknown because `runs-on` is dynamic;
...
Malformed or unreadable workflow content is represented with `unknown` or `unavailable` workflow state rather than treated as a passing workflow.
