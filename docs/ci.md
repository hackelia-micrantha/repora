...
`.github/workflows/release-tag.yml` is the preferred operator entrypoint. Its single privileged job has job-scoped `contents: write` and `actions: write`, requires an explicitly supplied release commit to be an exact full SHA, verifies that target is reachable from current `main`, validates the version heading from that target commit's own `CHANGELOG.md`, refuses existing tags, creates the tag without force, then explicitly dispatches `release.yml`. If the commit input is omitted, current `main` is used intentionally. Supplying the exact reviewed release commit is preferred so unrelated later changes on `main` cannot silently change the release target. The explicit dispatch is necessary because a tag push generated with the repository `GITHUB_TOKEN` does not itself recursively start another workflow.
