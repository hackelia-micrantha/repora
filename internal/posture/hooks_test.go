...
func TestCollectGitHubHooksDetectsCustomHooksAndCICoverage(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("collect hooks: %v", err)
	}
	if inventory.Manager.Value == nil || *inventory.Manager.Value != "custom" {
		t.Fatalf("manager = %#v", inventory.Manager)
	}
	if len(inventory.Entrypoints) != 1 || !boolValue(t, inventory.Entrypoints[0].Configured) || !boolValue(t, inventory.Entrypoints[0].NetworkLoaded) {
	...
	if len(inventory.RequiredChecks) != 1 || !boolValue(t, inventory.RequiredChecks[0].CICovered) {
		t.Fatalf("checks = %#v", inventory.RequiredChecks)
	}
	if !boolValue(t, inventory.BootstrapPresent) || !boolValue(t, inventory.BypassPresent) {
		t.Fatalf("docs facts = %#v %#v", inventory.BootstrapPresent, inventory.BypassPresent)
	}
}
...
func TestCollectGitHubHooksReportsMissingDocsAndCIMismatch(t *testing.T) {
	...
	if boolValue(t, inventory.BootstrapPresent) || boolValue(t, inventory.BypassPresent) {
		t.Fatalf("missing docs reported present")
	}
	if len(inventory.RequiredChecks) != 1 || boolValue(t, inventory.RequiredChecks[0].CICovered) {
		t.Fatalf("CI mismatch not represented: %#v", inventory.RequiredChecks)
	}
}
...
func TestCollectGitHubHooksUnavailableDoesNotInventHealthyFacts(t *testing.T) {
	...
	if inventory.Manager.State != StateUnavailable || inventory.ProfileDeclared.State != StateUnavailable {
		t.Fatalf("unavailable facts = %#v", inventory)
	}
}
...
func TestParseHooksProfileRejectsEscapingPath(t *testing.T) {
	...
	if err == nil {
		t.Fatal("escaping hooks path accepted")
	}
}
