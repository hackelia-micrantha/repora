...
func mirrorLocal(branch string, mirrorState status.State) MirrorLocalObservation {
	return MirrorLocalObservation{
		...
		CanonicalBranch: MirrorBranchObservation{
			Name:      "main",
			Available: true,
			Evidence:  Evidence{Source: "git.remote_head", Reference: "canonical"},
		},
		MirrorBranches: []MirrorBranchObservation{
			{
				Name:      branch,
				Available: true,
				Evidence:  Evidence{Source: "git.remote_head", Reference: "mirror-0"},
			},
		},
	}
}
...
func providerFacts() fakeMirrorProviderReader {
	...
	return fakeMirrorProviderReader{
		...
		obs: map[string]ReadObservation{
			"gitlab:acme/example": {
				Available: true,
				Evidence:  Evidence{Source: "fake.provider", Reference: "gitlab:acme/example"},
			},
			"github:acme/example": {
				Available: true,
				Evidence:  Evidence{Source: "fake.provider", Reference: "github:acme/example"},
			},
		},
	}
}
...
func TestCollectMirrorPostureInSync(t *testing.T) {
	repo := mirrorTestRepo()
	inventory, err := CollectMirrorPosture(
		context.Background(),
		config.Spec{Repos: []config.Repo{repo}},
		fakeMirrorLocalObserver{observation: mirrorLocal("main", status.StateEqual)},
		providerFacts(),
	)
	if err != nil {
		t.Fatal(err)
...
func TestCollectMirrorPostureDetectsDefaultBranchDrift(t *testing.T) {
	...
	inventory, err := CollectMirrorPosture(
		context.Background(),
		config.Spec{Repos: []config.Repo{repo}},
		fakeMirrorLocalObserver{observation: mirrorLocal("master", status.StateEqual)},
		providerFacts(),
	)
	if err != nil {
		t.Fatal(err)
...
func TestCollectMirrorPosturePreservesUnavailableProviderEvidence(t *testing.T) {
	...
	providers.obs["github:acme/example"] = ReadObservation{
		Available: false,
		Evidence:  Evidence{Source: "github.repository", Reference: "acme/example", Detail: "HTTP 403"},
	}
	inventory, err := CollectMirrorPosture(
		context.Background(),
		config.Spec{Repos: []config.Repo{repo}},
...
func TestCollectMirrorPostureDoesNotInventDriftWhenMirrorObservationFails(t *testing.T) {
	...
	local.MirrorBranches[0] = MirrorBranchObservation{
		Evidence: Evidence{Source: "git.remote_head", Reference: "mirror-0", Detail: "unavailable"},
	}
	local.Status.Mirrors[0].Commit = ""
	providers := providerFacts()
	providers.obs["github:acme/example"] = ReadObservation{
		Available: false,
		Evidence:  Evidence{Source: "github.repository", Reference: "acme/example"},
	}
	inventory, err := CollectMirrorPosture(
		context.Background(),
		config.Spec{Repos: []config.Repo{repo}},
		fakeMirrorLocalObserver{observation: local},
		providers,
	)
	if err != nil {
		t.Fatal(err)
