...
func (i MirrorInventory) Validate() error {
	...
	for ri, repo := range i.Repos {
		if err := validateMirrorRepository(ri, repo); err != nil {
			return err
		}
	}
	return nil
...
func validateMirrorRepository(index int, repo MirrorRepositoryFacts) error {
	if strings.TrimSpace(repo.ID) == "" || strings.TrimSpace(repo.UID) == "" {
		return fmt.Errorf("repo[%d] id and uid are required", index)
	}
	if repo.Mirrors == nil || repo.Evidence == nil {
		return fmt.Errorf("repo[%d] mirror and evidence arrays are required", index)
	}
	if strings.TrimSpace(repo.Canonical.Identity.Provider) == "" || strings.TrimSpace(repo.Canonical.Identity.Path) == "" {
		return fmt.Errorf("repo[%d] canonical identity is required", index)
	}
	...
	for mi, mirror := range repo.Mirrors {
		if err := validateMirrorTarget(index, mi, mirror); err != nil {
			return err
		}
	}
	return nil
...
func validateMirrorTarget(repoIndex, mirrorIndex int, mirror MirrorTargetFacts) error {
	...
	checks := []error{
		validateFact("cache_remote", mirror.CacheRemote),
		validateFact("default_branch", mirror.DefaultBranch),
		validateFact("default_branch_drift", mirror.DefaultBranchDrift),
		validateFact("commit", mirror.Commit),
		validateFact("divergence", mirror.Divergence),
		validateFact("ahead", mirror.Ahead),
		validateFact("behind", mirror.Behind),
		validateFact("visibility", mirror.Visibility),
		validateFact("current_actor_push_permission", mirror.CurrentActorPushPermission),
		validateFact("tag_drift", mirror.TagDrift),
		validateFact("release_drift", mirror.ReleaseDrift),
	}
	for _, err := range checks {
		if err != nil {
			return fmt.Errorf("repo[%d] mirror[%d]: %w", repoIndex, mirrorIndex, err)
		}
	}
	return nil
}
...
func validateMirrorEndpointFacts(prefix string, branch, commit, visibility Fact[string], push Fact[bool]) error {
	checks := []error{
		validateFact("default_branch", branch),
		validateFact("commit", commit),
		validateFact("visibility", visibility),
		validateFact("current_actor_push_permission", push),
	}
	for _, err := range checks {
		if err != nil {
