...
func TestBuildPlanRepresentsCreationOfEmptyREADME(t *testing.T) {
	configPath := writePlannerConfigAndTemplate(t, "", "repora")
	spec := configuredPlannerSpec("repo.repora", "repora", "templates/README.md.tmpl", nil)
	observer := &fakeREADMEObserver{byUID: map[string]READMEObservation{
		"repo.repora": {
			Branch:  "main",
	...
	return config.Spec{Repos: []config.Repo{
		{
			...
			Mirrors: []config.Endpoint{{Provider: "github", Path: "hackelia-micrantha/repora"}},
			Mode:    "mirror",
...
func writePlannerConfigAndTemplate(t *testing.T, template, id string) string {
	t.Helper()
	root := t.TempDir()
