...
// segments. Each segment is JSON quoted so line-ending-only changes such as
// CRLF to LF remain visible without embedding terminal control bytes.
func ReviewDiff(observed, desired []byte) (string, error) {
	...
	oldLines := splitExactLines(string(observed))
	newLines := splitExactLines(string(desired))
...
func splitExactLines(value string) []string {
	if value == "" {
		return nil
	}
	var lines []string
